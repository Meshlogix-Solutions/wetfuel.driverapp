...
@Component({
  ...
  template: `
<wf-mobile-shell title="Fuel delivery" subtitle="Generator GEN-04" backRoute="/meter">
  <main class="screen-body stack">
    <ion-card class="wf-card text-center">
      <ion-card-content>
        ...
        <div class="gauge" [style.--value]="percent + '%'"><div class="gauge-inner"><div><strong>{{ gallons }}</strong><div class="muted">of 250 gal</div></div></div></div>
    ...
    <div>
      ...
      <ion-range min="0" max="250" [(ngModel)]="gallons" [pin]="true"></ion-range>
    </div>
...
export class FuelingPage {
  ...
  get percent(): number { return Math.min(100, Math.round((Number(this.gallons) / 250) * 100)); }
  toggleFueling(): void { this.fueling = !this.fueling; }
}
