...
@Component({
  ...
  template: `
<wf-mobile-shell title="Confirm equipment" subtitle="QR scan successful" backRoute="/qr-scanner"><main class="screen-body stack">
  ...
  <ion-card class="wf-card warning-card"><ion-card-content><strong>Driver check</strong><p class="caption">Confirm the QR label and reference details match the physical asset before connecting the hose.</p><ion-item lines="none"><ion-checkbox slot="start" [(ngModel)]="confirmed"></ion-checkbox><ion-label class="ion-text-wrap"><strong>Equipment identity confirmed</strong><p class="caption">Equipment condition appears safe for fueling.</p></ion-label></ion-item></ion-card-content></ion-card>
...
export class EquipmentPage {
  ...
  confirm():void{const job=this.state.selectedJob();if(job)this.state.updateJob(job.id,'equipment_verified');void this.router.navigateByUrl('/meter');}
  mismatch():void{this.state.verifiedEquipment.set(null);void this.router.navigateByUrl('/qr-scanner');}
}
