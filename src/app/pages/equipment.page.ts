...
@Component({
  ...
  imports: [MobileShellComponent, LoaderComponent, IonCard, IonCardContent, IonIcon, IonItem, IonCheckbox, IonLabel, IonButton],
  template: `
<wf-mobile-shell title="Confirm equipment" subtitle="QR scan successful" [backRoute]="'/jobs/' + (state.selectedJob()?.id || '') + '/qr-scanner'"><main class="screen-body stack">
  ...
  <ion-button class="wf-button" expand="block" [disabled]="!confirmed || state.busy()" (click)="confirm()">@if(state.busy()){<wf-loader mode="button" />}Continue to meter connection</ion-button><ion-button class="wf-button" color="danger" fill="outline" expand="block" (click)="mismatch()">Equipment does not match</ion-button>
