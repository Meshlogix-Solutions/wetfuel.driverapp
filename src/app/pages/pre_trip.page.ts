...
<wf-mobile-shell title="Pre-job checklist" [subtitle]="'Job ' + (state.selectedJob()?.jobNumber ?? '')" [backRoute]="'/jobs/' + jobId + '/vehicle-confirmation'">
  <main class="screen-body stack">
    ...
    <ion-card class="wf-card"><ion-card-content><ion-item *ngFor="let item of items; let i = index" lines="full"><ion-checkbox slot="start" [(ngModel)]="checks[i]"></ion-checkbox><ion-label class="ion-text-wrap"><strong>{{ item[0] }}</strong><p class="caption">{{ item[1] }}</p></ion-label></ion-item></ion-card-content></ion-card>
