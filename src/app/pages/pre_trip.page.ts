...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, MobileShellComponent, LoaderComponent, IonCard, IonCardContent, IonItem, IonCheckbox, IonLabel, IonTextarea, IonButton],
  template: `
<wf-mobile-shell title="Pre-job checklist" [subtitle]="'Job ' + (state.selectedJob()?.jobNumber ?? '')" [backRoute]="'/jobs/' + jobId + '/vehicle-confirmation'" [showNav]="true">
  <main class="screen-body stack">
    ...
    <ion-button class="wf-button" expand="block" [disabled]="completedCount !== items.length || state.busy()" (click)="submit()">
      @if (state.busy()) { <wf-loader mode="button" /> }
      Complete checklist and start job
    </ion-button>
  </main>
</wf-mobile-shell>
  `,
