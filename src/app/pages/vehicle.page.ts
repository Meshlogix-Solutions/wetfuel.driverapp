...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MobileShellComponent, LoaderComponent, IonCard, IonCardContent, IonIcon, IonButton],
  template: `
<wf-mobile-shell title="Confirm vehicle" [subtitle]="'Job ' + (state.selectedJob()?.jobNumber ?? '')" [backRoute]="'/jobs/' + jobId" [showNav]="true">
  <main class="screen-body stack">
    @if (loading()) {
      <wf-loader mode="section" message="Loading vehicles..." />
    } @else {
    ...
    <ion-button class="wf-button" expand="block" [disabled]="loading() || state.busy() || !selected" (click)="confirm()">
      @if (state.busy()) { <wf-loader mode="button" /> }
      Confirm vehicle for this job
    </ion-button>
    }
  </main>
</wf-mobile-shell>
